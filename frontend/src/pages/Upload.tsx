    ...
    <>
      ...
      {!activeJobId && (
        <p className="value-body" style={{ marginBottom: "1.5rem" }}>
          Drop your image — we'll remove the background, flip it right, and hand it back in seconds, and ready for your listing, your socials, or your portfolio.
        </p>
      )}

      {activeJobId ? (
        <>
          <ResultPanel jobId={activeJobId} onDeleted={resetToUpload} />
          <button className="btn-upload visible" onClick={resetToUpload} style={{ marginTop: "1rem" }}>
            Process another <HiOutlineRefresh />
          </button>
        </>
      ) : (
        <>
